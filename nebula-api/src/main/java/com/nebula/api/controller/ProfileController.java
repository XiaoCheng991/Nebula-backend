...
public class ProfileController {
    ...
    public Result<UserProfileVO> getUserProfile(@RequestHeader("Authorization") String token) {
        try {
    ...
    public Result<Void> updateUserProfile(
            ...
            @Valid @RequestBody UserProfileVO profileVO
    ) {
        try {
            // 从token获取用户ID
            Long userId = authService.validateToken(token);
    ...
    public Result<Void> updateUserAvatar(
        ...
        try {
            ...
            log.info("更新用户头像成功: userId={}, avatarUrl={}",
                    userInfo.getId(), avatarRequest.getAvatarUrl());
