...
public interface LoginLogMapper extends BaseMapper<LoginLog> {
    ...
    int countFailedAttemptsByIp(@Param("ipAddress") String ipAddress, @Param("since") LocalDateTime since);

    /**
     * 查询用户的最近失败登录次数
     */
    @Select("SELECT COUNT(*) FROM login_logs WHERE user_id = #{userId} AND status = 'failed' AND login_at > #{since} AND deleted = 0")
    int countFailedAttemptsByUser(@Param("userId") Long userId, @Param("since") LocalDateTime since);
    ...
    List<java.util.Map<String, Object>> countByLoginType(@Param("since") LocalDateTime since);

    /**
     * 清理旧的登录日志
     */
    @Select("UPDATE login_logs SET deleted = 1 WHERE login_at < #{beforeTime} AND deleted = 0")
    int cleanupOldLogs(@Param("beforeTime") LocalDateTime beforeTime);
}
