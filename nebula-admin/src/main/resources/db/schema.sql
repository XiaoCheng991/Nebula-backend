...
CREATE TABLE IF NOT EXISTS sys_users (
    ...
    avatar_size BIGINT,
    bio TEXT,
    online_status VARCHAR(20) DEFAULT 'offline' CHECK (online_status IN ('online', 'offline', 'busy', 'away')),
...
CREATE INDEX IF NOT EXISTS idx_sys_users_account_status ON sys_users(account_status);
CREATE INDEX IF NOT EXISTS idx_sys_users_online_status ON sys_users(online_status);
...
COMMENT ON COLUMN sys_users.avatar_name IS '头像文件名称';
COMMENT ON COLUMN sys_users.avatar_url IS '头像URL（MinIO）';
COMMENT ON COLUMN sys_users.avatar_size IS '头像文件大小（字节）';
COMMENT ON COLUMN sys_users.bio IS '个人简介';
COMMENT ON COLUMN sys_users.online_status IS '在线状态';
COMMENT ON COLUMN sys_users.account_status IS '账号状态（0-禁用，1-启用）';
COMMENT ON COLUMN sys_users.last_login_at IS '最后登录时间';
COMMENT ON COLUMN sys_users.last_seen_at IS '最后活跃时间';
